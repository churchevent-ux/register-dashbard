...
const Sidebar = () => {
  ...
  { icon: "👦", text: "Kids (8-12)", path: "/admin/kids" },
  { icon: "🧑", text: "Teens (13-18)", path: "/admin/teens" },
  { icon: "🎓", text: "Students", path: "/admin/users" },
    ...
    { icon: "�", text: "Cash Payment", path: "/admin/cash-payment" },
    ...
    // { icon: "🗂️", text: "History", path: "/admin/history" },
      {/* Teams removed as requested */}
  ];

  // Track dashboard tab for sidebar highlighting
  const [dashboardTab, setDashboardTab] = useState(window.__dashboardTab || 'all');
  useEffect(() => {
    const handler = (e) => {
      setDashboardTab(e.detail || 'all');
      window.__dashboardTab = e.detail || 'all';
    };
